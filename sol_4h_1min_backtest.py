"""
sol_4h_1min_backtest.py — SOL/USDT backtest using 4H candles for signals,
1-minute data for precise outcome resolution.

Same pipeline as sol_1min_backtest.py except:
  - Engulfings detected on 4H candles (resampled from 1H CSV)
  - Highlander S/R levels built on 4H bars
  - HTF trend uses Daily SMA20/50 (resampled from 1H)
  - 1-min data still used for exact activation minute and outcome
"""
...
from sol_1min_backtest import (
    ...
    FILTER_DIST_ATR, BLOCK_DIST_ATR,
)
from highlander import run_indicator, DIR_UP, DIR_DOWN
from test_new_symbols import P_THRESHOLD
...
# 4H-specific features (subset — no 4H HTF since signals ARE on 4H)
FEATURES_4H = [
...
CAT_FEATURES_4H = ["htf_trend", "btc_trend", "session", "day"]


def build_4h_trades(df_1h: pd.DataFrame, df_1m: pd.DataFrame, btc_df: pd.DataFrame,
    ...
    # ── Daily HTF trend (SMA20/50 on daily close) ─────────────────────────────
    daily = df[["close"]].resample("1D", label="left").last().dropna()
    daily["sma20"] = daily["close"].rolling(20).mean()
    daily["sma50"] = daily["close"].rolling(50).mean()
    daily["t"] = np.where(daily["sma20"] > daily["sma50"],  1,
                 np.where(daily["sma20"] < daily["sma50"], -1, 0))
    didx = np.clip(daily.index.searchsorted(df.index, side="right") - 1, 0, len(daily) - 1)
    df["htf_label"] = np.where(daily["t"].values[didx] ==  1, "Bullish",
                      np.where(daily["t"].values[didx] == -1, "Bearish", "Range"))
    ...
    # ── 1-min timestamps for outcome resolution ───────────────────────────────
    ...
    for i in range(1, len(df)):
        ...
        for is_bull in ([True] * int(bull) + [False] * int(bear)):
            ...
            act_4h_open_ns = np.int64(df.index[act_bar].value)
            act_4h_end_ns  = np.int64((df.index[act_bar] + pd.Timedelta(hours=4)).value)
            i_m_start = int(np.searchsorted(m1_ts, act_4h_open_ns, side="left"))
            i_m_end   = int(np.searchsorted(m1_ts, act_4h_end_ns,  side="left"))
            ...
            # ── Resolve outcome using 1-min data ──────────────────────────────
...
def add_confluence_4h(ds: pd.DataFrame, df_1h: pd.DataFrame) -> pd.DataFrame:
    """Add Highlander confluence features using 4H levels."""
    from test_new_symbols import add_confluence
    df_4h = (df_1h[["open","high","low","close","volume"]]
             .resample("4h", label="left")
             .agg({"open":"first","high":"max","low":"min","close":"last","volume":"sum"})
             .dropna())
    return add_confluence(ds, df_4h)


def walk_forward_4h(ds: pd.DataFrame):
    from catboost import CatBoostClassifier
    ...
    cat_idx = [FEATURES_4H.index(c) for c in CAT_FEATURES_4H]
    ...
    while train_end <= end:
        ...
        X_tr = tr[FEATURES_4H].values.astype(object)
        y_tr = (tr["outcome"] == "win").astype(int).values
        X_te = te[FEATURES_4H].values.astype(object)
    ...
    all_n  = len(ds)
    filt_n = m.sum()
    all_r  = ds["r_strategy_D_trail"].sum()
    ...
    span_months = (ds["activation_time"].max() - ds["activation_time"].min()).days / 30.44
    ...
    print(f"\n{'═'*60}")
    print(f"  SOL/USDT  4H signals + 1-min resolution  |  walk-forward OOS")
    print(f"{'═'*60}")
    print(f"  All signals    : {all_n:,}   R={all_r:.1f}")
    ...
    print(f"  Total R        : {filt_r:.2f}R  over {span_months:.1f} months")
    print(f"  R/month        : {filt_r/span_months:.3f}R")
...
if __name__ == "__main__":
    ...
    # ── Build trades ──────────────────────────────────────────────────────────
    print(f"\nBuilding trades (4H signals, 1-min resolution) …")
    ds = build_4h_trades(df_1h, df_1m, btc_df)
    if ds.empty:
